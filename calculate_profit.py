...
def parse_size(item):
    ...
    for name, label in labels.items():
        m = re.search(label + r'\s*(?:[:：=]|は)?\s*約?\s*(\d{1,4}(?:\.\d+)?)\s*(?:cm|センチ)?', s, re.I)
        if m:
            values[name] = _num(m.group(1))
    if len(values) == 3:
        return values
    m = re.search(r'(?:幅|横幅)\s*(?:[:：=]|は)?\s*約?\s*(\d{1,4}(?:\.\d+)?)\s*(?:cm|センチ)?\s*[×xX]\s*(?:奥行(?:き)?)\s*(?:[:：=]|は)?\s*約?\s*(\d{1,4}(?:\.\d+)?)\s*(?:cm|センチ)?\s*[×xX]\s*(?:高さ|縦幅)\s*(?:[:：=]|は)?\s*約?\s*(\d{1,4}(?:\.\d+)?)', s, re.I)
    if m:
        return {'length': _num(m.group(1)), 'width': _num(m.group(2)), 'height': _num(m.group(3))}
    m = re.search(r'(\d{2,4}(?:\.\d+)?)\s*[×xX]\s*(\d{2,4}(?:\.\d+)?)\s*[×xX]\s*(\d{2,4}(?:\.\d+)?)\s*(?:cm|センチ)?', s, re.I)
    if m:
...
def main():
    ...
    # Match by stable Jimoty URL first. Title matching is only a fallback because
    # titles can collide (e.g. several listings simply called "本棚").
    by_url = {str(x.get('source_url')): x for x in checked if x.get('source_url')}
    ...
    for item in candidates:
        ...
        result = by_url.get(str(item.get('url')))
        if not result:
        ...
        confidence = '低'
        if count >= 5 and len(title) >= 4:
            confidence = '中'
        if count >= 10 and len(title) >= 6 and similarity >= 25:
            confidence = '高'
        ...
        rank_profit = int(profit * 0.5) if profit is not None and confidence == '低' else profit
        rows.append({
            'title': title,
            ...
            'mercari_query': result.get('query'),
    ...
    for r in rows[:10]:
        print(r['title'], 'query=', r['mercari_query'], 'size=', r['size'], 'profit=', r['estimated_profit'], 'shipping=', r['shipping'], 'confidence=', r['confidence'])
